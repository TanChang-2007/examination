...
function App() {
  ...
  return (
    <div className="flex bg-gradient-to-r from-[#f0c1fd] to-[#aaffff] w-full h-screen text-3xl overflow-auto">
      <div className='absolute felx flex-auto items-center pt-64 pl-[32rem]' >{(Math.random(quotes))}</div>
      <div className='flex flex-1 items-end justify-center pb-36'>
      <a href="reload(true)" class="continue"><button className=' fixed'><Icon icon="ion:reload" width="30" height="30" /></button></a>
      </div>
      <div className='flex flex-row p-3 gap-3 right-0 top-0 justify-end absolute'>
          <div className=''>{moment().format('ddd')}</div>
          <div className=''>{moment().format('D')}</div>
          <div className=''>{moment().format('MMM')}</div>
      </div>
    </div>
  )
}
